function st_textnvl() {
	...
	for (j=0; j<array_length(backlog); j++) {
		if backlog[j].type == "normal" {
			...
			draw_text(x-width/2-40, y+(fontsize)+(lb+nvlline)*(fontsize*2), backlog[j].displayname);
	...
	if msg[talkpos].type == "normal" {
			...
			draw_set_halign(fa_right);
			draw_text(x-width/2-40, y+(fontsize)+(lb+nvlline)*(fontsize*2), msg[talkpos].displayname);
			draw_set_halign(fa_left);
			draw_set_color(c_white);
		}
	atsound = 0;
